...
# rep_list = goal_door['Left_Door'][0][0]['image']
# text_list = goal_door['Left_Door'][0][0]['text']
rep_list = []
text_list = []
for states in goal_door['Left_Door']:
    for state in states:
        rep_list.append(state['image'])
        text_list.append(state['text'])
        
for states in goal_door['Right_Door']:
    for state in states:
        rep_list.append(state['image'])
        text_list.append(state['text'])
...
#   nconv_mean = nconv.mean(0)
#   # print('after',nconv_mean.shape)
#   conv1_tmp = nconv - nconv_mean#.unsqueeze(1)
#   # print('after2',conv1_tmp.shape)
#   # conv1_sq = torch.sqrt(1/nconv_mean.shape[0] * torch.sum((torch.sum(conv1_tmp**2))**2, 0))
#   # print('after3', torch.sum(conv1_tmp**2,1).shape)
#   conv1_sq = torch.sqrt(1/nconv_mean.shape[0] * torch.sum((torch.sum(conv1_tmp**2, 1))))
  
#   return conv1_sq


def get_dispersion(nconv):
    nconv = nconv.view(nconv.shape[0], -1)  # Flatten to (N, M)
    overall_mean = nconv.mean()            # Scalar mean over all elements
    centered = nconv - overall_mean
    variance = (centered ** 2).mean()      # Mean squared deviation
    dispersion = torch.sqrt(variance)      # Standard deviation
    return dispersion    
...
for relu_ in network_activations:
  ...
  meanofmeans = torch.sum(torch.stack(mean_list))
  denominator = 0
  count = 0
  print(len(mean_list))
  for i in range(len(mean_list)):
    for j in range(len(mean_list)):
        if i != j:
          denominator += torch.sqrt(torch.sum((mean_list[i] - mean_list[j])**2))
  ...
  for i in range(relu_.shape[0]):
    if i%(360)==0:
      numerator += get_dispersion(relu_[i:i+(360)])
  numerator = numerator/(360)

  dispersions.append((numerator/denominator).item())
  # dispersions.append((denominator/numerator).item())
  dispersions_n.append(numerator.item())
  dispersions_d.append(denominator.item())
