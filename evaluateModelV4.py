...
choosen_action = {'0':[], '1':[], '2':[], '6':[]}
for goal_keys in goal_door.keys():
    for states in goal_door[goal_keys][:-1]:
        ...
        the_choosen_action = str(int(stats.mode(actions).mode))
        for state in states:
            choosen_action[the_choosen_action].append(state)
...
for action_key in choosen_action.keys():
    for state in choosen_action[action_key]:
        image = state['image']
        text = state['text']
        obs = {'image': image, 
               'direction': np.int64(1),
        ...
        conv1[action_key].append(hidden[0][0].flatten())
        conv2[action_key].append(hidden[0][1].flatten())
        conv3[action_key].append(hidden[0][2].flatten())
        conv4[action_key].append(hidden[0][3].flatten())
        conv5[action_key].append(hidden[0][4].flatten())
        conv6[action_key].append(hidden[0][5].flatten())
        conv7[action_key].append(hidden[0][6].flatten())
        conv8[action_key].append(hidden[0][7].flatten())
        actionsl[action_key].append(action)
    
        l1[action_key].append(hidden[1][0].flatten())
        l2[action_key].append(hidden[1][1].flatten())
        l3[action_key].append(hidden[1][2].flatten())
      
        
dispersions = []
mean_list = []

network_activations = [conv1, conv2, conv3, conv4, conv5, conv6, conv7, conv8,
                       l1, l2, l3,]
                       # torch.tensor(actionsl).to(device).float()]


dispersions = []
for layer_ in network_activations:
    mean_list = {'0':torch.zeros(layer_['0'][0].flatten().shape), '1':torch.zeros(layer_['0'][0].flatten().shape), 
                 '2':torch.zeros(layer_['0'][0].flatten().shape), '6':torch.zeros(layer_['0'][0].flatten().shape),}
    
    for action_key in choosen_action.keys():
        mean_list[action_key] = torch.mean(torch.stack(layer_[action_key]))
        
    variance_list = {'0':0, '1':0, '2':0, '6':0}
    for action_key in choosen_action.keys():
        for values in layer_[action_key]:
            variance_list[action_key] += torch.sum((values.flatten() - mean_list[action_key])**2)
        
        variance_list[action_key] = variance_list[action_key]/len(layer_[action_key])

    # mean_variance = torch.tensor(variance_list)/2
    # # print(variance_list)
    # # print(mean_variance)

    expected_dispersion = 0
    for action_key in choosen_action.keys():
        expected_dispersion += torch.sqrt(variance_list[action_key])
    expected_dispersion = expected_dispersion/4
    # print(expected_dispersion)
    # print(torch.sqrt(mean_variance))

    mean_pairwise_dist = 0
    count = 0
    for i in choosen_action.keys():
        for j in choosen_action.keys():
            if i!=j:
